...
@interface Ule_Scanner ()<AVCaptureMetadataOutputObjectsDelegate>
 ...
 *  相机对焦
 */
CGFloat const kUleFocalPointOfInterestX = 0.5;
CGFloat const kUleFocalPointOfInterestY = 0.5;

@implementation Ule_Scanner
...
- (void)startScanningWithResultBlock:(void (^)(NSArray *codes))resultBlock {
    ...
    NSAssert([Ule_Scanner cameraIsPresent], @"没有相机.");
    NSAssert(![Ule_Scanner scanningIsProhibited], @"没有权限.");
...
- (void)flipCamera {
    if (self.isScanning) {
        if (self.camera == UleCameraFront) {
            self.camera = UleCameraBack;
        } else {
            self.camera = UleCameraFront;
        }
    }
}
...
- (AVCaptureDevice *)newCaptureDeviceWithCamera:(UleCamera)camera {
    ...
    if ([mNewCaptureDevice lockForConfiguration:&lockError] == YES) {
        ...
        if ([mNewCaptureDevice respondsToSelector:@selector(isFocusPointOfInterestSupported)] &&
            ...
            mNewCaptureDevice.focusPointOfInterest = CGPointMake(kUleFocalPointOfInterestX, kUleFocalPointOfInterestY);
        }
...
- (AVCaptureDevicePosition)devicePositionForCamera:(UleCamera)camera {
    switch (camera) {
        case UleCameraFront:
...
- (void)setCamera:(UleCamera)camera {
